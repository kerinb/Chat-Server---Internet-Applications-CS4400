...
public class ClientThread extends Thread {
   	...
   	public ClientThread(Socket socket){
   		...
   		catch(IOException e){ErrorAndPrintHandler.printError(e.getMessage(), "Occurred when creating new client thread");}
   	}
   	...
   	public void run(){
   			try{
   				...
   				if((ChatServer.numLiveThreads.get() == 0) && (ChatServer.getRunningValue() == false)){ChatServer.shutServerDown();}
   	...
   	private void handlerErrorProcessingRequest(ErrorMessages invalidrequest, RequestTypeNode requestTypeNode) {
   		String errorMessage = String.format(ResponceFromServer.ERROR.getValue(), invalidrequest.getValue(),invalidrequest.getDescription());
   		try{this.connectedClient.getPrintWriter().write(errorMessage);}
   		catch(Exception e){ErrorAndPrintHandler.printError(e.getMessage(), "Occurred when handling request error");}
   	}
   	...
   	private synchronized void disconnect(RequestTypeNode requestTypeNode)  {
   		...
   		try {ChatServer.removeClientFromServer(requestTypeNode, connectedClient);} 
   		catch (Exception e) {ErrorAndPrintHandler.printError(e.getMessage(), "Occurred When disconnecting from Server");}
   	}
   	...
   	private synchronized void killService(RequestTypeNode requestTypeNode) {
   		...
   		if(!ChatServer.getServerSocket().isClosed()){handleKillServiceError(requestTypeNode, ErrorMessages.KillService);}
   	}
   	...
   	private void handleKillServiceError(RequestTypeNode requestTypeNode, ErrorMessages killservice) {
   		...
   		try{this.connectedClient.getPrintWriter().write(errorMessageToPrint);}
   		catch(Exception e){ErrorAndPrintHandler.printError(e.getMessage(), "Couldnt communicate failed killserver error to client");}
   	}
   	
   	private void helo(RequestTypeNode requestTypeNode){
   		try{writeToClient(makeHeloResponce(requestTypeNode));}
   		catch(Exception e){ErrorAndPrintHandler.printError(e.getMessage(), "Occurred when saying helo");}
   	}
   	
   	private static String makeHeloResponce(RequestTypeNode requestTypeNode) {
   		return String.format(ResponceFromServer.HELO.getValue(), requestTypeNode.getRequestsReceivedFromClient().get(0).split(HELO_ID)[1].replaceAll("\n", ""),
   	...
   	private void leaveChatRoom(RequestTypeNode requestTypeNode) {
   		...
   		try{
   			...
   			if(leaveChatRoom != null){
   				...
   				if(clientInChatRoom(leaveChatRoom)){leaveChatRoom.removeClientRecord(this.connectedClient, requestTypeNode);}
   			}
   			ErrorAndPrintHandler.printString("NULL CHATROOM VALUE TO LEAVE"); 
   		}
   		catch(Exception e){ErrorAndPrintHandler.printError(e.getMessage(), "Occurred when leaving chatroom"); }
   	}
   	...
   	private boolean clientInChatRoom(ChatRoom chatRoom) {
   		for(ConnectedClient connectedClient : chatRoom.getListOfAllConnectedClients()){
   			if(connectedClient.getSocket().equals(this.connectedClient.getSocket())){return true;}
   		}
   		return false;
   	}
   	...
   	private void joinChatRoom(RequestTypeNode requestTypeNode) {
   		...
   		try{
   			...
   			if(this.joinId == UNKNOWN_JOIN_ID){this.joinId = ChatServer.nextClientId.getAndIncrement();}
   			if(requestedChatRoomToJoin == null){
   				...
   				ErrorAndPrintHandler.printString(String.format("ChatRoom: %s alread exist... Adding Client: %s to chatroom", requestTypeNode.getChatRoomId(), requestTypeNode.getName(), requestTypeNode.getChatRoomId()));
   				try{requestedChatRoomToJoin.addClientRecordToChatRoom(this.connectedClient, requestTypeNode);}
   				catch(Exception e){
   		...
   		catch(Exception e){ErrorAndPrintHandler.printError(e.getMessage(), "ErrorJoing Chatroom");}
   	}
   
   	private void writeJoinRequestAndBroadcast(ChatRoom requestedChatRoomToJoin, RequestTypeNode requestTypeNode) {
   		String messageToClient = String.format(ResponceFromServer.JOIN.getValue(), requestedChatRoomToJoin.getChatRoomId(), 
   				ChatServer.serverIP,ChatServer.serverPort, requestedChatRoomToJoin.getChatRoomRef(),  this.joinId);
  		writeToClient(messageToClient);
   		String messageToBroadCast = String.format("%s has joined chatroom %s", requestTypeNode.getName(),requestTypeNode.getChatRoomId());
   	...
   	private synchronized void writeToClient(String messageToBroadCast) {
   		...
   		catch(Exception e){ErrorAndPrintHandler.printString("failed to send message to client.....\n");}
   	}
   	...
   	public RequestTypeNode getInfoFromClient(RequestType requestType, List<String> message) throws IOException {
   		switch (requestType) {
   		case JOIN_CHATROOM:
   			return new RequestTypeNode(message.get(3).split(CLIENT_NAME_ID, 0)[1],message.get(0).split(JOIN_CHATROOM_ID, 0)[1], message, requestType);
   		case CHAT:
   			this.joinId = Integer.parseInt(message.get(1).split(JOIN_ID_IDENTIFIER, 0)[1]);
   			return new RequestTypeNode(message.get(2).split(CLIENT_NAME_ID, 0)[1],message.get(0).split(CHAT_ID, 0)[1], message, requestType);
   		case LEAVE_CHATROOM:
   			this.joinId = Integer.parseInt(message.get(1).split(JOIN_ID_IDENTIFIER, 0)[1]);
   			return new RequestTypeNode(message.get(2).split(CLIENT_NAME_ID, 0)[1],message.get(0).split(LEAVE_CHATROOM_ID, 0)[1], message, requestType);
   		case DISCONNECT:
   	...
   	private List<String> getAsArrayList(String fromClient) {
   		...
   		for(String line: lines){linesToReturn.add(line);}
